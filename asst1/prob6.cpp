...
/**
 * Helper argument passed into each thread
 * Each thread updates the cluster assignment for a chunk of data points
 * (e.g. computes the distance with every centroid and keeps the min)
 * 
...
void workerThread(HelperArgs *const thread_args) {
    ...
    for (int m = thread_args->startM; m < thread_args->endM; m++) {
      ...
      double localMinDist = 1e30; 
      int localClusterAssignment = -1; 

      for (int k = thread_args->start; k < thread_args->end; k++) { 
...
void computeAssignments(WorkerArgs *const args) {
  ...
  for (int m =0; m < args->M; m++) {
    ...
    for (int i=0; i<numThreads; i++) {
          ...
          helper_args[i].data = &args->data[0]; 
          helper_args[i].clusterAssignments = &args->clusterAssignments[0]; 
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  std::cout << "computeAssignments: " << assign_comp << std::endl;
  std::cout << "computeCentroids: " << centroids_comp << std::endl;
  std::cout << "computeCost: " << cost_comp << std::endl;
